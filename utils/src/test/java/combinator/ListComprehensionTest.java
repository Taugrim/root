...
class ListComprehensionTest {
    ...
    void of2() {
       var l= ListComprehension.of(
                List::of,
                List.of(
               ()->  List.of("a1").stream().map(q -> Map.entry("z", q)).collect(Collectors.toList()),
               ()-> List.of("q1", "q2").stream().map(q -> Map.entry("q", q)).collect(Collectors.toList()),
               ()-> List.of("w1", "w2", "w3").stream().map(q -> Map.entry("w", q)).collect(Collectors.toList()),
               ()->  List.of("e1", "e2").stream().map(q -> Map.entry("e", q)).collect(Collectors.toList()),
               ()->   List.of("r1").stream().map(q -> Map.entry("r", q)).collect(Collectors.toList()))
